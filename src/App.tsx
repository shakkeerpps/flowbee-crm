...
export default function App() {
  ...
  // Drilldown Campaign Selector State for Reports Tab
  const [reportCampaignSelection, setReportCampaignSelection] = useState<string>('ALL');
  ...
  const analyticsEngine = useMemo(() => {
    ...
    leads.forEach(l => {
      ...
      const agent = l.assignedSalesName || 'Unassigned';

      // Aggregate Campaign
      if (!campaignStats[camp]) campaignStats[camp] = { total: 0, won: 0, adSets: {} };
      campaignStats[camp].total += 1;
      if (l.status === 'Closed Won') campaignStats[camp].won += 1;

      if (!campaignStats[camp].adSets[adSet]) campaignStats[camp].adSets[adSet] = { total: 0, won: 0 };
      campaignStats[camp].adSets[adSet].total += 1;
      if (l.status === 'Closed Won') campaignStats[camp].adSets[adSet].won += 1;

      // Aggregate AdSet
      const adSetKey = `${camp} ➔ ${adSet}`;
      if (!adSetStats[adSetKey]) adSetStats[adSetKey] = { campaign: camp, total: 0, won: 0 };
      adSetStats[adSetKey].total += 1;
      if (l.status === 'Closed Won') adSetStats[adSetKey].won += 1;

      // Aggregate Sales Rep
      if (!agentStats[agent]) agentStats[agent] = { total: 0, won: 0 };
      agentStats[agent].total += 1;
      if (l.status === 'Closed Won') agentStats[agent].won += 1;
    });

    const sortedCampaigns = Object.entries(campaignStats).sort((a, b) => b[1].won - a[1].won || b[1].total - a[1].total);
    const sortedAdSets = Object.entries(adSetStats).sort((a, b) => b[1].won - a[1].won || b[1].total - a[1].total);
    const sortedAgents = Object.entries(agentStats).sort((a, b) => b[1].won - a[1].won || b[1].total - a[1].total);

    return {
      campaignStats,
      bestCampaign: sortedCampaigns[0] ? { name: sortedCampaigns[0][0], ...sortedCampaigns[0][1] } : null,
      worstCampaign: sortedCampaigns.length > 1 ? { name: sortedCampaigns[sortedCampaigns.length - 1][0], ...sortedCampaigns[sortedCampaigns.length - 1][1] } : null,

      bestAdSet: sortedAdSets[0] ? { name: sortedAdSets[0][0].split(' ➔ ')[1], campaign: sortedAdSets[0][1].campaign, ...sortedAdSets[0][1] } : null,
      worstAdSet: sortedAdSets.length > 1 ? { name: sortedAdSets[sortedAdSets.length - 1][0].split(' ➔ ')[1], campaign: sortedAdSets[sortedAdSets.length - 1][1].campaign, ...sortedAdSets[sortedAdSets.length - 1][1] } : null,

      bestAgent: sortedAgents[0] ? { name: sortedAgents[0][0], ...sortedAgents[0][1] } : null,
      worstAgent: sortedAgents.length > 1 ? { name: sortedAgents[sortedAgents.length - 1][0], ...sortedAgents[sortedAgents.length - 1][1] } : null,
    };
  }, [leads]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const { data, error } = await supabase.from('sales_reps').select('*').eq('email', loginEmail.trim()).eq('password', loginPassword.trim()).single();
    if (error || !data) alert('❌ Invalid Email or Password!');
    else if (data.status === 'Inactive') alert('🔴 Your account is inactive. Please contact Administrator.');
    else { setCurrentUser(data); localStorage.setItem('crm_user', JSON.stringify(data)); }
    setLoading(false);
  };

  const handleLogout = () => { setCurrentUser(null); localStorage.removeItem('crm_user'); };
  ...
  const handleDeleteAgent = async (agentId: string, agentName: string) => {
    ...
    if (assignedLeads.length > 0) {
      alert(`⚠️ Cannot delete "${agentName}"! There are ${assignedLeads.length} active leads assigned to this agent.`);
      return;
    }
  ...
  const handleUpdateAgent = async (e: React.FormEvent) => {
    ...
    if (!error) { setEditingAgent(null); fetchData(); }
  };
  ...
  const handleAddAgent = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('sales_reps').insert([{
      name: agentName, email: agentEmail, phone: agentPhone, status: 'Active', role: 'sales_executive'
    }]);
    if (!error) { setAgentName(''); setAgentEmail(''); setAgentPhone(''); setIsAddAgentModalOpen(false); fetchData(); }
  };
  ...
  const handleUpdateActivity = async (e: React.FormEvent) => {
    ...
    if (!error) { setEditingActivity(null); fetchData(); }
  };
  ...
  const handleScheduleActivity = async (e: React.FormEvent) => {
    ...
    if (!error) { setActivityDesc(''); setActiveActivityLead(null); fetchData(); }
  };

  const handleCompleteWorkflow = async (actionType: 'Done' | 'DoneAndNext' | 'CancelAndNext') => {
    if (!completingActivity) return;
    const targetStatus = actionType === 'CancelAndNext' ? 'Cancelled' : 'Completed';
    const { error } = await supabase.from('lead_activities').update({
      status: targetStatus,
    ...
    if (!error) {
      ...
      if (actionType === 'DoneAndNext' || actionType === 'CancelAndNext') {
        if (parentLead) setActiveActivityLead(parentLead);
      }
      fetchData();
    }
  };
  ...
  const handleUpdateSpend = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('daily_spend_logs').update({
      date: editingSpend.date, platform: editingSpend.platform, spend_amount: amount, clicks: clicks, cpc: cpc
    }).eq('id', editingSpend.id);
    if (!error) { setEditingSpend(null); fetchData(); }
  };
  ...
  const handleUpdateLead = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('leads').update({
      customer_name: editingLead.customerName, phone: editingLead.phone, email: editingLead.email,
      platform: editingLead.platform, remark: editingLead.remark, date_added: editingLead.dateAdded,
      campaign_name: editingLead.campaignName || null, ad_set: editingLead.adSet || null
    }).eq('id', editingLead.id);

    if (!error) { setEditingLead(null); fetchData(); }
  };
    ...
    if (!error) {
      ...
      if (newStatus === 'Closed Won') {
        sendEmailNotification('WON', {
          leadName: lead?.customerName, repName: lead?.assignedSalesName, phone: lead?.phone, agentEmail: agentObj?.email || 'crm@flowbee.io', campaignName: lead?.campaignName
        });
      } else if (newStatus === 'Closed Lost') {
        sendEmailNotification('LOST', {
          leadName: lead?.customerName, repName: lead?.assignedSalesName, agentEmail: agentObj?.email || 'crm@flowbee.io', campaignName: lead?.campaignName
        });
      } else {
        sendEmailNotification('STATUS_CHANGE', {
          leadName: lead?.customerName, leadPhone: lead?.phone, repName: lead?.assignedSalesName, status: newStatus, agentEmail: agentObj?.email || 'crm@flowbee.io', campaignName: lead?.campaignName
        });
      }
      fetchData();
    }
  ...
  const visibleActivities = activities.filter(a => {
    ...
    return currentUser?.role === 'sales_executive' ? parentLead?.assignedSalesId === currentUser.id : true;
  });
  ...
  // Export functions
  const exportToExcel = () => {
    if (filteredLeads.length === 0) return alert("No data available to export!");
    const headers = ["Customer Name", "Phone", "Email", "Platform", "Campaign", "AdSet", "Sales Rep", "Stage", "Date Added"];
    const rows = filteredLeads.map(l => [
      `"${l.customerName}"`, `"${l.phone}"`, `"${l.email || 'N/A'}"`, `"${l.platform}"`,
      `"${l.campaignName || 'N/A'}"`, `"${l.adSet || 'N/A'}"`, `"${l.assignedSalesName || 'Unassigned'}"`, `"${l.status}"`, `"${l.dateAdded}"`
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map(e => e.join(","))].join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Leads_Export_${todayStr}.csv`);
  ...
  const exportToPDF = () => {
    ...
    const tableRows = filteredLeads.map(l => [l.customerName, l.phone, l.campaignName || 'N/A', l.adSet || 'N/A', l.assignedSalesName || 'Unassigned', l.status, l.dateAdded]);
    autoTable(doc, { startY: 28, head: [tableColumn], body: tableRows, theme: 'striped', headStyles: { fillColor: [37, 99, 235] }, styles: { fontSize: 8 } });
    doc.save(`Leads_Report_${todayStr}.pdf`);
  };
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="flex items-center space-x-2 border rounded-xl p-3 bg-slate-50">
                ...
                <input type="email" required placeholder="admin@flowbee.io" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="bg-transparent outline-none text-xs w-full font-semibold" />
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center space-x-2 border rounded-xl p-3 bg-slate-50">
                ...
                <input type="password" required placeholder="••••••••" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} className="bg-transparent outline-none text-xs w-full font-semibold" />
              </div>
            </div>
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-xs transition shadow-md">{loading ? 'Authenticating...' : 'Sign In to Dashboard'}</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="h-screen w-screen flex flex-col md:flex-row bg-slate-100 text-slate-800 overflow-hidden">
      ...
      <header className="md:hidden bg-slate-900 text-white flex items-center justify-between px-4 py-3 shrink-0 z-30 shadow-md">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={() => setIsChangePasswordModalOpen(true)} className="p-2 bg-slate-800 rounded-lg text-slate-300"><KeyRound size={16} /></button>
          <button onClick={handleLogout} className="p-2 bg-slate-800 rounded-lg text-rose-400"><LogOut size={16} /></button>
        </div>
      </header>

      {/* SIDEBAR */}
      <aside className={`bg-slate-900 text-slate-300 transition-all duration-300 flex flex-col justify-between h-full z-50 shrink-0 ${isMobileMenuOpen ? 'fixed inset-y-0 left-0 w-72 shadow-2xl flex' : 'hidden md:flex'} ${isSidebarExpanded ? 'md:w-64' : 'md:w-20'}`}>
        <div>
          ...
          <nav className="p-3 space-y-2">
            <button onClick={() => changeTab('dashboard')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition ${activeTab === 'dashboard' ? 'bg-blue-600 text-white font-semibold' : 'hover:bg-slate-800 text-slate-400'}`}>
              ...
              <span className={`${!isSidebarExpanded && 'md:hidden'}`}>Dashboard</span>
            </button>
            ...
            <button onClick={() => changeTab('leads')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition ${activeTab === 'leads' ? 'bg-blue-600 text-white font-semibold' : 'hover:bg-slate-800 text-slate-400'}`}>
              ...
              <span className={`${!isSidebarExpanded && 'md:hidden'}`}>Leads Pipeline</span>
            </button>
            ...
            <button onClick={() => changeTab('followups')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition ${activeTab === 'followups' ? 'bg-blue-600 text-white font-semibold' : 'hover:bg-slate-800 text-slate-400'}`}>
              ...
              <span className={`${!isSidebarExpanded && 'md:hidden'}`}>Follow-ups</span>
            </button>
            ...
            {currentUser.role === 'admin' && (
              <>
                ...
                <button onClick={() => changeTab('agents')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition ${activeTab === 'agents' ? 'bg-blue-600 text-white font-semibold' : 'hover:bg-slate-800 text-slate-400'}`}>
                  ...
                  <span className={`${!isSidebarExpanded && 'md:hidden'}`}>Sales Team</span>
                </button>
                ...
                <button onClick={() => changeTab('reports')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-xl transition ${activeTab === 'reports' ? 'bg-blue-600 text-white font-semibold' : 'hover:bg-slate-800 text-slate-400'}`}>
                  ...
                  <span className={`${!isSidebarExpanded && 'md:hidden'}`}>Reports & Analytics</span>
                </button>
              </>
            )}
          </nav>
        ...
        <div className="p-4 border-t border-slate-800 space-y-2 mb-12 md:mb-0">
          <button onClick={() => setIsChangePasswordModalOpen(true)} className="w-full flex items-center justify-center space-x-2 bg-slate-800 hover:bg-slate-700 text-slate-300 py-2 rounded-xl text-xs transition"><KeyRound size={14} /><span className={`${!isSidebarExpanded && 'md:hidden'}`}>Change Password</span></button>
          <button onClick={handleLogout} className="w-full flex items-center justify-center space-x-2 bg-rose-950/60 hover:bg-rose-900 text-rose-300 py-2 rounded-xl text-xs transition"><LogOut size={14} /><span className={`${!isSidebarExpanded && 'md:hidden'}`}>Sign Out</span></button>
        </div>
      </aside>
      ...
      <main className="flex-1 h-full overflow-y-auto p-4 md:p-8 pb-20 md:pb-8">
        ...
        {/* 📊 DASHBOARD TAB */}
        {activeTab === 'dashboard' && (() => {
          const scopedSpendLogs = spendLogs.filter(s => (!dashFromDate || s.date >= dashFromDate) && (!dashToDate || s.date <= dashToDate));
          const scopedLeads = visibleLeads.filter(l => (!dashFromDate || (l.dateAdded && l.dateAdded >= dashFromDate)) && (!dashToDate || (l.dateAdded && l.dateAdded <= dashToDate)));
            ...
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Realtime lead stats, CPL, ad cost & top performer leaderboard</p>
                </div>
                ...
                <div className="flex items-center space-x-2 text-xs bg-slate-50 p-2 rounded-xl border w-full md:w-auto justify-between md:justify-start">
                  ...
                  <input type="date" value={dashFromDate} onChange={e => setDashFromDate(e.target.value)} className="bg-transparent font-bold text-blue-600 outline-none text-xs" />
                  <span className="text-slate-300">•</span>
                  <span className="font-bold text-slate-500 text-[10px]">To:</span>
                  <input type="date" value={dashToDate} onChange={e => setDashToDate(e.target.value)} className="bg-transparent font-bold text-blue-600 outline-none text-xs" />
                </div>
              </div>

              {/* 🌟 TOP CAMPAIGN & ADSET HIGHLIGHT CARDS */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gradient-to-r from-purple-600 to-indigo-700 text-white p-4 rounded-2xl shadow-md flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-purple-200 flex items-center space-x-1">
                      <Trophy size={12} /> <span>Best Performing Campaign</span>
                    </span>
                    <h3 className="text-lg font-black mt-1">{analyticsEngine.bestCampaign?.name || 'N/A'}</h3>
                    <p className="text-xs text-purple-100 font-semibold">{analyticsEngine.bestCampaign?.won || 0} Deals Won ({analyticsEngine.bestCampaign?.total || 0} Total Leads)</p>
                  </div>
                  <div className="p-3 bg-white/20 rounded-2xl"><Megaphone size={28} /></div>
                </div>

                <div className="bg-gradient-to-r from-blue-600 to-teal-600 text-white p-4 rounded-2xl shadow-md flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-blue-200 flex items-center space-x-1">
                      <Award size={12} /> <span>Best Performing AdSet</span>
                    </span>
                    <h3 className="text-lg font-black mt-1">{analyticsEngine.bestAdSet?.name || 'N/A'}</h3>
                    <p className="text-xs text-blue-100 font-semibold">Campaign: <strong>{analyticsEngine.bestAdSet?.campaign || 'N/A'}</strong> ({analyticsEngine.bestAdSet?.won || 0} Won)</p>
                  </div>
                  <div className="p-3 bg-white/20 rounded-2xl"><Layers size={28} /></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
                {currentUser.role === 'admin' && (
                  <>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase flex items-center space-x-1"><DollarSign size={12} className="text-emerald-600" /> <span>Total Spend</span></p>
                      <h3 className="text-lg md:text-2xl font-black text-slate-800">₹{totalSpendSum.toLocaleString('en-IN')}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase flex items-center space-x-1"><MousePointerClick size={12} className="text-blue-600" /> <span>Total Clicks</span></p>
                      <h3 className="text-lg md:text-2xl font-black text-blue-600">{totalClicksSum.toLocaleString('en-IN')}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase flex items-center space-x-1"><Calculator size={12} className="text-purple-600" /> <span>Avg CPC</span></p>
                      <h3 className="text-lg md:text-2xl font-black text-purple-600">₹{avgCPCOverall}</h3>
                    </div>
                  </>
                )}
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
                  <p className="text-[10px] font-bold text-slate-500 uppercase flex items-center space-x-1"><Target size={12} className="text-indigo-600" /> <span>Leads</span></p>
                ...
                {currentUser.role === 'admin' && (
                  <div className="bg-gradient-to-br from-emerald-600 to-teal-800 text-white p-4 rounded-2xl shadow-md space-y-1 col-span-2 lg:col-span-1">
                    <p className="text-[10px] font-bold uppercase text-emerald-100 flex items-center space-x-1"><TrendingUp size={12} /> <span>Avg CPL</span></p>
                    <h3 className="text-lg md:text-2xl font-black">₹{avgCPLOverall}</h3>
                  </div>
                )}
              </div>

              {/* FOLLOWUPS & CLOSED DEALS */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                  <div className="space-y-1">
                    <span className="text-[11px] font-bold text-slate-500 uppercase flex items-center space-x-1"><Clock size={13} className="text-amber-500" /> <span>Pending Follow-ups</span></span>
                  ...
                  <div className="flex flex-col items-end space-y-1 text-xs">
                    <span className="bg-rose-100 text-rose-700 font-bold px-2.5 py-0.5 rounded-lg border border-rose-200 text-[10px]">⚠️ Overdue: {overdueActsCount}</span>
                    <span className="bg-amber-100 text-amber-800 font-bold px-2.5 py-0.5 rounded-lg border border-amber-200 text-[10px]">📌 Today: {todayActsCount}</span>
                  </div>
                </div>

                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                  <div className="space-y-1">
                    <span className="text-[11px] font-bold text-slate-500 uppercase flex items-center space-x-1"><Flame size={13} className="text-emerald-600" /> <span>Closed Deals (Won)</span></span>
                    <h3 className="text-xl md:text-2xl font-black text-emerald-600">{wonLeadsCount} Deals</h3>
                  </div>
                  <span className="bg-emerald-50 text-emerald-700 p-2.5 rounded-2xl border border-emerald-100"><CheckCircle2 size={22} /></span>
                </div>
              </div>
              ...
              <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                ...
                <ChartsDashboard spendLogs={scopedSpendLogs} leads={scopedLeads} salesReps={salesReps} activities={visibleActivities} />
              </div>
            </div>
          );
        })()}
          ...
          <div className="space-y-4 md:space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <button onClick={exportToPDF} className="flex-1 sm:flex-none flex items-center justify-center space-x-1 bg-rose-600 text-white font-semibold px-3 py-2 rounded-xl text-xs hover:bg-rose-700 transition"><Download size={14} /> <span>PDF</span></button>
                <button onClick={exportToExcel} className="flex-1 sm:flex-none flex items-center justify-center space-x-1 bg-emerald-600 text-white font-semibold px-3 py-2 rounded-xl text-xs hover:bg-emerald-700 transition"><FileSpreadsheet size={14} /> <span>Excel</span></button>
                <button onClick={() => setIsAddLeadModalOpen(true)} className="w-full sm:w-auto flex items-center justify-center space-x-1.5 bg-blue-600 text-white font-semibold px-3.5 py-2 rounded-xl text-xs hover:bg-blue-700 shadow-md transition"><PlusCircle size={16} /> <span>+ Add Lead</span></button>
              </div>
            </div>
                  ...
                  <div key={l.id} className="bg-white p-3.5 md:p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3 transition hover:border-blue-300">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b pb-3">
                      <div>
                        <div className="flex items-center justify-between sm:justify-start space-x-2">
                          ...
                          <div className="flex items-center space-x-1 ml-auto sm:ml-2">
                            <button onClick={() => setEditingLead(l)} title="Edit Lead" className="p-1 hover:bg-blue-50 text-blue-600 rounded-lg transition"><Edit2 size={15} /></button>
                            <button onClick={() => handleDeleteLead(l.id)} title="Delete Lead" className="p-1 hover:bg-rose-50 text-rose-600 rounded-lg transition"><Trash2 size={15} /></button>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-1 text-[11px] md:text-xs text-slate-500">
                          ...
                          <div className="flex items-center space-x-1">
                            <span>Rep:</span>
                            {currentUser.role === 'admin' ? (
                              <select value={l.assignedSalesId || ''} onChange={(e) => handleReassignLead(l.id, e.target.value)} className={`font-bold p-0.5 px-1 rounded-lg border outline-none text-[11px] ${!l.assignedSalesId ? 'bg-amber-100 text-amber-800' : 'bg-blue-50 text-blue-700'}`}>
                      ...
                      <div className="flex items-center space-x-1.5 overflow-x-auto py-1 max-w-full">
                        {STAGES.map(s => (
                          <button key={s} onClick={() => handleStageChange(l.id, s)} className={`text-[11px] md:text-xs px-2.5 py-1 rounded-xl transition shrink-0 ${l.status === s ? getStageBadgeStyle(s) : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{s}</button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                      <div className="flex flex-wrap items-center gap-2">
                        <a href={`tel:${l.phone}`} className="flex items-center space-x-1 bg-blue-50 text-blue-600 px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-blue-100"><Phone size={13} /> <span>Call</span></a>
                        <a href={`https://wa.me/${l.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="flex items-center space-x-1 bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-emerald-100"><MessageCircle size={13} /> <span>WhatsApp</span></a>
                        <button onClick={() => setActiveActivityLead(l)} className="flex items-center space-x-1 bg-purple-50 text-purple-700 px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-purple-100"><Calendar size={13} /> <span>+ Schedule</span></button>
                      </div>

                      <div className="flex items-center justify-between sm:justify-end space-x-2">
                        {getLeadFollowupBadge(leadActs)}
                        <button onClick={() => setExpandedLeadId(isExpanded ? null : l.id)} className="flex items-center space-x-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded-xl text-xs transition"><FileText size={13} /> <span>{isExpanded ? 'Hide Logs ▲' : `View Logs (${leadActs.length}) ▼`}</span></button>
                      </div>
                    </div>
                        ...
                        {leadActs.map(act => (
                          <div key={act.id} className="bg-white p-2.5 rounded-lg border text-slate-700 space-y-1">
                            <div className="flex justify-between font-semibold">
                              <span className="text-blue-600">[{act.activity_type}] {act.description}</span>
                              <span className="text-[10px] px-2 py-0.5 rounded bg-slate-100">{act.status} ({act.follow_up_date})</span>
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        {activeTab === 'followups' && (
          <div className="space-y-4 md:space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-500">Filter and manage scheduled activities</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {filteredFollowups.map(act => {
                ...
                return (
                  <div key={act.id} className="p-4 rounded-2xl border bg-white shadow-sm space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">Date: <strong>{act.follow_up_date}</strong> at <strong>{act.follow_up_time || 'N/A'}</strong></p>
                      </div>
                      <div className="flex space-x-1">
                        <button onClick={() => setEditingActivity(act)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={14} /></button>
                        <button onClick={() => handleDeleteActivity(act.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-lg"><Trash2 size={14} /></button>
                      </div>
                    </div>
                    <p className="text-xs bg-slate-50 p-2.5 rounded-xl border text-slate-700"><strong>Purpose:</strong> {act.description}</p>
                    <button onClick={() => setCompletingActivity(act)} className="w-full bg-blue-600 text-white text-xs font-semibold py-2.5 rounded-xl hover:bg-blue-700 transition">Update Status & Next Step</button>
                  </div>
                );
              })}
            </div>
          ...
          <div className="space-y-4 md:space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-500">Manage agent profiles and status</p>
              </div>
              <button onClick={() => setIsAddAgentModalOpen(true)} className="flex items-center space-x-1.5 bg-blue-600 text-white font-semibold px-3.5 py-2 rounded-xl text-xs"><PlusCircle size={16} /> <span>+ Add Agent</span></button>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {salesReps.map(agent => (
                <div key={agent.id} className="bg-white p-4 rounded-2xl border shadow-sm space-y-3">
                  <div className="flex justify-between items-start">
                    <h4 className="font-bold text-slate-900">{agent.name}</h4>
                    <button onClick={() => setEditingAgent(agent)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={15} /></button>
                  </div>
                  <p className="text-xs text-slate-500">{agent.email || 'No email'}</p>
                  <button onClick={() => handleToggleAgentStatus(agent)} className={`px-3 py-1 rounded-xl text-xs font-bold ${agent.status !== 'Inactive' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-600'}`}>
                    {agent.status !== 'Inactive' ? '🟢 Active' : '🔴 Inactive'}
                  </button>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'reports' && currentUser.role === 'admin' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-500">Track best & worst performing marketing campaigns, ad sets, and sales executives</p>
            </div>

            {/* 🌟 LEADERBOARDS: BEST VS WORST */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-emerald-50/60 border border-emerald-200 p-4 rounded-2xl space-y-3">
                <h3 className="text-sm font-black text-emerald-900 flex items-center space-x-2"><Flame size={18} className="text-emerald-600" /><span>🌟 Top Performers (Best)</span></h3>
                <div className="space-y-2 text-xs">
                  <div className="bg-white p-3 rounded-xl border border-emerald-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">BEST SALES REP</span><strong className="text-slate-800 text-sm">{analyticsEngine.bestAgent?.name || 'N/A'}</strong></div>
                    <span className="bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.bestAgent?.won || 0} Won</span>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-emerald-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">BEST CAMPAIGN</span><strong className="text-purple-700 text-sm">{analyticsEngine.bestCampaign?.name || 'N/A'}</strong></div>
                    <span className="bg-purple-100 text-purple-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.bestCampaign?.won || 0} Won</span>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-emerald-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">BEST ADSET</span><strong className="text-indigo-700 text-sm">{analyticsEngine.bestAdSet?.name || 'N/A'}</strong><p className="text-[10px] text-slate-500">Campaign: {analyticsEngine.bestAdSet?.campaign || 'N/A'}</p></div>
                    <span className="bg-indigo-100 text-indigo-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.bestAdSet?.won || 0} Won</span>
                  </div>
                </div>
              </div>

              <div className="bg-rose-50/60 border border-rose-200 p-4 rounded-2xl space-y-3">
                <h3 className="text-sm font-black text-rose-900 flex items-center space-x-2"><AlertTriangle size={18} className="text-rose-600" /><span>⚠️ Needs Improvement (Lowest)</span></h3>
                <div className="space-y-2 text-xs">
                  <div className="bg-white p-3 rounded-xl border border-rose-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">LOWEST SALES REP</span><strong className="text-slate-800 text-sm">{analyticsEngine.worstAgent?.name || 'N/A'}</strong></div>
                    <span className="bg-rose-100 text-rose-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.worstAgent?.won || 0} Won</span>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-rose-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">LOWEST CAMPAIGN</span><strong className="text-slate-700 text-sm">{analyticsEngine.worstCampaign?.name || 'N/A'}</strong></div>
                    <span className="bg-rose-100 text-rose-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.worstCampaign?.won || 0} Won</span>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-rose-100 flex justify-between items-center">
                    <div><span className="text-[10px] text-slate-400 font-bold block">LOWEST ADSET</span><strong className="text-slate-700 text-sm">{analyticsEngine.worstAdSet?.name || 'N/A'}</strong><p className="text-[10px] text-slate-500">Campaign: {analyticsEngine.worstAdSet?.campaign || 'N/A'}</p></div>
                    <span className="bg-rose-100 text-rose-800 px-2.5 py-1 rounded-lg font-bold">{analyticsEngine.worstAdSet?.won || 0} Won</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b pb-3">
                ...
                <select value={reportCampaignSelection} onChange={e => setReportCampaignSelection(e.target.value)} className="border rounded-xl p-2 text-xs bg-slate-50 font-bold text-purple-700 outline-none">
              ...
              <div className="space-y-4">
                {Object.entries(analyticsEngine.campaignStats)
                  .filter(([camp]) => reportCampaignSelection === 'ALL' || reportCampaignSelection === camp)
                  .map(([campName, campData]) => (
                    <div key={campName} className="border rounded-2xl p-4 bg-slate-50/50 space-y-3">
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-[10px] font-bold text-purple-600 uppercase">Campaign</span>
                          <h4 className="text-base font-black text-slate-900">{campName}</h4>
                        </div>
                        <div className="flex space-x-2 text-xs font-bold">
                          <span className="bg-blue-100 text-blue-800 px-2.5 py-1 rounded-lg">Leads: {campData.total}</span>
                          <span className="bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded-lg">Won: {campData.won}</span>
                        </div>
                      </div>

                      <div className="bg-white rounded-xl border p-3 space-y-2">
                        <p className="text-[11px] font-bold text-slate-500 uppercase flex items-center space-x-1"><Layers size={12} className="text-indigo-600" /><span>AdSets Under "{campName}"</span></p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                          {Object.entries(campData.adSets).map(([adSetName, adStats]) => (
                            <div key={adSetName} className="bg-slate-50 p-2.5 rounded-lg border text-xs space-y-1">
                              <span className="font-bold text-slate-800 block truncate">{adSetName}</span>
                              <div className="flex justify-between text-[11px] text-slate-500">
                                <span>Total: <strong>{adStats.total}</strong></span>
                                <span className="text-emerald-600 font-bold">Won: {adStats.won}</span>
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Create New Lead</h3>
                <button onClick={() => setIsAddLeadModalOpen(false)}><X size={20} /></button>
              </div>

              <div className="space-y-3">
                <input type="date" required value={leadCustomDate} onChange={e => setLeadCustomDate(e.target.value)} className="w-full border rounded-xl p-2 text-xs font-semibold text-blue-600" />
                <input type="text" placeholder="Customer Name *" required value={leadName} onChange={e => setLeadName(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs outline-none" />
                ...
                <select value={leadPlatform} onChange={e => setLeadPlatform(e.target.value as PlatformName)} className="w-full border rounded-xl p-2.5 text-xs outline-none">
                  {PLATFORMS.map(p => <option key={p} value={p}>{p}</option>)}
                </select>

                <textarea placeholder="Initial Remark Details..." value={leadRemark} onChange={e => setLeadRemark(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs h-20 outline-none" />
              </div>

              <div className="pt-2 border-t flex flex-col gap-2">
                <button onClick={() => handleSaveLead(false)} className="w-full bg-blue-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-blue-700 transition">Save Lead</button>
                {currentUser.role === 'admin' && (
                  <button onClick={() => handleSaveLead(true)} className="w-full bg-emerald-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-emerald-700 transition">Save & Assign (Round-Robin)</button>
                )}
              </div>
            </div>
          </div>
        ...
        {editingLead && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Edit Lead Details</h3>
                <button onClick={() => setEditingLead(null)}><X size={20} /></button>
              </div>

              <form onSubmit={handleUpdateLead} className="space-y-3">
                <input type="date" required value={editingLead.dateAdded} onChange={e => setEditingLead({ ...editingLead, dateAdded: e.target.value })} className="w-full border rounded-xl p-2 text-xs font-semibold text-blue-600" />
                <input type="text" required value={editingLead.customerName} onChange={e => setEditingLead({ ...editingLead, customerName: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs outline-none" />
                ...
                <select value={editingLead.platform} onChange={e => setEditingLead({ ...editingLead, platform: e.target.value as PlatformName })} className="w-full border rounded-xl p-2.5 text-xs outline-none">
                  {PLATFORMS.map(p => <option key={p} value={p}>{p}</option>)}
                </select>

                <textarea value={editingLead.remark || ''} onChange={e => setEditingLead({ ...editingLead, remark: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs h-20 outline-none" />

                <div className="flex gap-2 pt-2 border-t">
                  <button type="submit" className="flex-1 bg-emerald-600 text-white font-semibold py-2 rounded-xl text-xs hover:bg-emerald-700">Save Changes</button>
                  <button type="button" onClick={() => setEditingLead(null)} className="flex-1 bg-slate-100 py-2 rounded-xl text-xs">Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {editingAgent && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Edit Sales Agent</h3>
                <button onClick={() => setEditingAgent(null)}><X size={20} /></button>
              </div>

              <form onSubmit={handleUpdateAgent} className="space-y-3">
                <input type="text" required value={editingAgent.name} onChange={e => setEditingAgent({ ...editingAgent, name: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs font-semibold" />
                <input type="email" value={editingAgent.email || ''} onChange={e => setEditingAgent({ ...editingAgent, email: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs" />
                <input type="text" value={editingAgent.phone || ''} onChange={e => setEditingAgent({ ...editingAgent, phone: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs" />
                <select value={editingAgent.status || 'Active'} onChange={e => setEditingAgent({ ...editingAgent, status: e.target.value as any })} className="w-full border rounded-xl p-2.5 text-xs font-semibold text-blue-700">
                  <option value="Active">🟢 Active</option>
                  <option value="Inactive">🔴 Inactive</option>
                </select>

                <div className="flex gap-2 pt-2 border-t">
                  <button type="submit" className="flex-1 bg-blue-600 text-white font-semibold py-2 rounded-xl text-xs hover:bg-blue-700">Save Changes</button>
                  <button type="button" onClick={() => setEditingAgent(null)} className="flex-1 bg-slate-100 py-2 rounded-xl text-xs">Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {editingSpend && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Edit Daily Spend Log</h3>
                <button onClick={() => setEditingSpend(null)}><X size={20} /></button>
              </div>

              <form onSubmit={handleUpdateSpend} className="space-y-3">
                <input type="date" required value={editingSpend.date} onChange={e => setEditingSpend({ ...editingSpend, date: e.target.value })} className="w-full border rounded-xl p-2 text-xs font-semibold text-blue-600" />
                <select value={editingSpend.platform} onChange={e => setEditingSpend({ ...editingSpend, platform: e.target.value as PlatformName })} className="w-full border rounded-xl p-2.5 text-xs outline-none">
                  {PLATFORMS.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
                <input type="number" required value={editingSpend.spendAmount} onChange={e => setEditingSpend({ ...editingSpend, spendAmount: Number(e.target.value) })} className="w-full border rounded-xl p-2.5 text-xs" />
                <input type="number" required value={editingSpend.clicks} onChange={e => setEditingSpend({ ...editingSpend, clicks: Number(e.target.value) })} className="w-full border rounded-xl p-2.5 text-xs" />

                <div className="flex gap-2 pt-2 border-t">
                  <button type="submit" className="flex-1 bg-emerald-600 text-white font-semibold py-2 rounded-xl text-xs hover:bg-emerald-700">Save Changes</button>
                  <button type="button" onClick={() => setEditingSpend(null)} className="flex-1 bg-slate-100 py-2 rounded-xl text-xs">Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {editingActivity && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Edit Follow-up Task</h3>
                <button onClick={() => setEditingActivity(null)}><X size={20} /></button>
              </div>

              <form onSubmit={handleUpdateActivity} className="space-y-3">
                <select value={editingActivity.activity_type} onChange={e => setEditingActivity({ ...editingActivity, activity_type: e.target.value as any })} className="w-full border rounded-xl p-2.5 text-xs">
                ...
                <textarea required value={editingActivity.description} onChange={e => setEditingActivity({ ...editingActivity, description: e.target.value })} className="w-full border rounded-xl p-2.5 text-xs h-20 outline-none" />
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" required value={editingActivity.follow_up_date || ''} onChange={e => setEditingActivity({ ...editingActivity, follow_up_date: e.target.value })} className="w-full border rounded-xl p-2 text-xs font-semibold text-blue-600" />
                  <input type="time" required value={editingActivity.follow_up_time || ''} onChange={e => setEditingActivity({ ...editingActivity, follow_up_time: e.target.value })} className="w-full border rounded-xl p-2 text-xs" />
                </div>

                <div className="flex gap-2 pt-2 border-t">
                  <button type="submit" className="flex-1 bg-blue-600 text-white font-semibold py-2 rounded-xl text-xs hover:bg-blue-700">Save Changes</button>
                  <button type="button" onClick={() => setEditingActivity(null)} className="flex-1 bg-slate-100 py-2 rounded-xl text-xs">Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {activeActivityLead && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Schedule Follow-up Task</h3>
              ...
              <form onSubmit={handleScheduleActivity} className="space-y-3">
                <select value={activityType} onChange={e => setActivityType(e.target.value as any)} className="w-full border rounded-xl p-2.5 text-xs outline-none">
                  <option value="Call">📞 Call Log</option>
                  <option value="Email">✉️ Email Sent</option>
                  <option value="WhatsApp">💬 WhatsApp Chat</option>
                  <option value="Meeting">🤝 Meeting Scheduled</option>
                </select>
                <textarea placeholder="Reason for follow-up..." required value={activityDesc} onChange={e => setActivityDesc(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs h-20 outline-none" />
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" required value={activityFollowUpDate} onChange={e => setActivityFollowUpDate(e.target.value)} className="w-full border rounded-xl p-2 text-xs" />
                  <input type="time" required value={activityFollowUpTime} onChange={e => setActivityFollowUpTime(e.target.value)} className="w-full border rounded-xl p-2 text-xs" />
                </div>
                <button type="submit" className="w-full bg-blue-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-blue-700 transition">Schedule Task</button>
              </form>
            </div>
          </div>
        )}
        ...
        {completingActivity && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Update Follow-up Outcome</h3>
                <button onClick={() => setCompletingActivity(null)}><X size={20} /></button>
              </div>

              <div className="space-y-3">
                <textarea placeholder="Enter discussion outcome..." value={completionNotes} onChange={e => setCompletionNotes(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs h-20 outline-none" />
                <div className="pt-2 flex flex-col gap-2">
                  <button onClick={() => handleCompleteWorkflow('Done')} className="w-full bg-emerald-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-emerald-700 transition">Done (Close Task)</button>
                  <button onClick={() => handleCompleteWorkflow('DoneAndNext')} className="w-full bg-blue-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-blue-700 transition">Done & Next Activity</button>
                  <button onClick={() => handleCompleteWorkflow('CancelAndNext')} className="w-full bg-rose-600 text-white font-semibold py-2.5 rounded-xl text-xs hover:bg-rose-700 transition">Cancel & Next Activity</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {isAddSpendModalOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Log Daily Spend</h3>
        ...
        {isAddAgentModalOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
              <div className="flex justify-between items-center border-b pb-3">
                <h3 className="font-bold text-slate-800 text-sm">Add Sales Agent</h3>
                <button onClick={() => setIsAddAgentModalOpen(false)}><X size={20} /></button>
              </div>

              <form onSubmit={handleAddAgent} className="space-y-3">
                <input type="text" placeholder="Agent Name *" required value={agentName} onChange={e => setAgentName(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs outline-none" />
                <input type="email" placeholder="Email Address" value={agentEmail} onChange={e => setAgentEmail(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs outline-none" />
                <input type="text" placeholder="Phone Number" value={agentPhone} onChange={e => setAgentPhone(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs outline-none" />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-5 max-w-md w-full space-y-4 shadow-xl">
            ...
            <form onSubmit={handleChangePassword} className="space-y-3">
              <input type="password" required placeholder="Enter new password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full border rounded-xl p-2.5 text-xs outline-none font-bold" />
              <div className="flex gap-2 pt-2 border-t">
